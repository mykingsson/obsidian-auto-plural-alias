...
__export(main_exports, {
  default: () => AutoPluralAliasPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian = require("obsidian");
var pluralize = __toESM(require_pluralize());
var AutoPluralAliasPlugin = class extends import_obsidian.Plugin {
  async onload() {
    console.log("Loading Auto Plural Alias Plugin");
    this.registerEvent(
      this.app.vault.on("create", async (file) => {
        if (file instanceof import_obsidian.TFile && file.extension === "md" && file.basename !== "Untitled") {
  ...
  async addPluralAlias(file) {
    const fileName = file.basename;
    const isPlural2 = pluralize.isPlural(fileName);
    const aliasForm = isPlural2 ? pluralize.singular(fileName) : pluralize.plural(fileName);
    if (aliasForm.toLowerCase() === fileName.toLowerCase()) {
      return;
    }
    await this.app.fileManager.processFrontMatter(file, (fm) => {
      let aliases = Array.isArray(fm.aliases) ? [...fm.aliases] : [];
